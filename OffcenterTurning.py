...
from varsetOps import setVarsetValue, getVarsetValue, getVarsetInt, setVarsetInt
	...
	def Activated(self):
		...
		class OffcenterTurningPanel:
			...
			def __init__(self):
				...
				self.top_angle_input.editingFinished.connect(lambda: self.update_values())
				self.cylinder_height_input = QtWidgets.QLineEdit(str(getVarsetValue(self.varset, "CylinderHeight")))
				self.cylinder_height_input.editingFinished.connect(lambda: self.update_values())
				self.cylinder_radius_input = QtWidgets.QLineEdit(str(getVarsetValue(self.varset, "CylinderRadius")))
				self.cylinder_radius_input.editingFinished.connect(lambda: self.update_values())
				self.top_radius_input = QtWidgets.QLineEdit(str(getVarsetValue(self.varset, "TopRadius")))
				self.top_radius_input.editingFinished.connect(lambda: self.update_values())
				self.bottom_radius_input = QtWidgets.QLineEdit(str(getVarsetValue(self.varset, "BottomRadius")))
				self.bottom_radius_input.editingFinished.connect(lambda: self.update_values())
				self.num_points_input = QtWidgets.QLineEdit(str(getVarsetInt(self.varset, "NumberPoints")))
				self.num_points_input.editingFinished.connect(lambda: self.change_num_points())
				self.cutting_radius_input = QtWidgets.QLineEdit(str(getVarsetValue(self.varset, "CuttingRadius")))
				self.cutting_radius_input.editingFinished.connect(lambda: self.update_values())
				...
				cutting_radius_row = QtWidgets.QHBoxLayout()
				cutting_radius_row.addWidget(QtWidgets.QLabel("Cutting Radius:"))
				cutting_radius_row.addWidget(self.cutting_radius_input)
				text_box_layout.addLayout(cutting_radius_row)				
				...
				# Add delete sketches button
				self.button_delete = QtWidgets.QPushButton("Delete Sketches")
				self.button_delete.clicked.connect(self.bt_delete_clicked)
			...
			def change_num_points(self):
				...
				if sketch is not None:
					self.update_values()
					self.bt_delete_clicked()
					self.bt_add_sketches_clicked()
				else:
					self.update_values()
			...
			def bt_add_sketches_clicked(self):
				...
				top_sketch.setExpression('.Placement.Base.z', 'BowlVariables.CylinderHeight')
				...
				bottom_sketch.addConstraint(Sketcher.Constraint('Radius', bottom_cylinder_circle_idx, getVarsetValue(self, "CylinderRadius")))
				bottom_sketch.addConstraint(Sketcher.Constraint('Coincident', bottom_cylinder_circle_idx, 3, -1, 1))
				bottom_sketch.setExpression('Constraints[2]', 'BowlVariables.CylinderRadius')  

				top_sketch.addConstraint(Sketcher.Constraint('Radius', top_cylinder_circle_idx, getVarsetValue(self, "CylinderRadius")))
				top_sketch.addConstraint(Sketcher.Constraint('Coincident', top_cylinder_circle_idx, 3, -1, 1))
				top_sketch.setExpression('Constraints[2]', 'BowlVariables.CylinderRadius')                                
				...
				if num_points > 0:
					...
					for i, geo in enumerate(bottom_sketch.Geometry):
						if geo.TypeId == "Part::GeomPoint":
							...
							obj.AttachmentSupport = [(bottom_sketch, prefix), (top_sketch, prefix)]
							obj.MapPathParameter = 0.0
							obj.MapMode = 'TwoPointLine'
							obj.recompute()

							obj2 = doc.addObject('Part::DatumPlane', 'DatumPlane')
							obj2.AttachmentOffset = App.Placement(App.Vector(0, 0, 50), App.Rotation(0, 0, 0))
							obj2.MapReversed = False
							obj2.AttachmentSupport = [(bottom_sketch, prefix), (obj, '')]
							obj2.MapPathParameter = 0.0
							obj2.MapMode = 'NormalToEdge'
							obj2.Visibility = False
							
							obj2.recompute()

							plane_sketch = doc.addObject("Sketcher::SketchObject", "Plane_Sketch001")
							plane_sketch.MapMode = "FlatFace"
							plane_sketch.AttachmentSupport = [(obj2, '')]
							plane_sketch.recompute()

							plane_sketch_circle = Part.Circle(App.Vector(0, 0, 0), App.Vector(0, 0, 1), getVarsetValue(self, "CuttingRadius"))
							plane_circle = plane_sketch.addGeometry(plane_sketch_circle, False)
							plane_sketch.addConstraint(Sketcher.Constraint('Radius', plane_circle, getVarsetValue(self, "CuttingRadius")))
							plane_sketch.setExpression('Constraints[0]', 'BowlVariables.CuttingRadius')  
							plane_sketch.addConstraint(Sketcher.Constraint('Coincident', plane_circle, 3, -1, 1))
							

							an_extrude = doc.addObject('Part::Extrusion', 'Extrude')
							#extrude = an_extrude
							an_extrude.Base = plane_sketch
							an_extrude.DirMode = "Normal"
							an_extrude.DirLink = None
							an_extrude.LengthFwd = 300
							an_extrude.LengthRev = 0.0
							an_extrude.Solid = True
							an_extrude.Reversed = True
							an_extrude.Symmetric = False
							an_extrude.TaperAngle = 0.0
							an_extrude.TaperAngleRev = 0.0
							#plane_sketch.Visibility = False
							doc.recompute()

					a_cylinder = doc.addObject('Part::Cylinder', 'Cylinder')
					a_cylinder.setExpression('Radius', 'BowlVariables.CylinderRadius')
					a_cylinder.setExpression('Height', 'BowlVariables.CylinderHeight')
					a_cylinder.Visibility = False
					extrude_list = []
					for obj in doc.Objects:
						if obj.Label.startswith("Extrude"):
							extrude_list.append(obj.Name)
			...
			def update_values(self):
				...
				setVarsetValue(self, "CylinderHeight", float(self.cylinder_height_input.text()))
				setVarsetValue(self, "CylinderRadius", float(self.cylinder_radius_input.text()))
				setVarsetValue(self, "TopRadius", float(self.top_radius_input.text()))  
				setVarsetValue(self, "BottomRadius", float(self.bottom_radius_input.text()))
				setVarsetValue(self, "BottomAngle", float(self.bottom_angle_input.text()))
				setVarsetValue(self, "TopAngle", float(self.top_angle_input.text()))
				setVarsetInt(self, "NumberPoints", int(self.num_points_input.text()))
				setVarsetValue(self, "CuttingRadius", float(self.cutting_radius_input.text()))
				doc = App.ActiveDocument
				doc.recompute()
